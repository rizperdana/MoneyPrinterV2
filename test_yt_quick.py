#!/usr/bin/env python3
"""
Quick test to verify URL extraction after upload - YouTube focused
"""
...
youtube.metadata = {
    "title": "Test YT - " + datetime.now().strftime("%Y%m%d %H:%M"),
...
if yt_success:
    ...
    if "youtube.com/watch?v=" in yt_url:
        print("✅ URL contains video ID!")
    else:
        print("⚠️ URL may not contain video ID")
else:
...
# Save result
with open(
    "/home/anon/Projects/experiment/MoneyPrinterV2/yt_test_result.json", "w"
) as f:
    json.dump({"success": yt_success, "url": yt_url}, f, indent=2)
